"""Point every tracker script at the ONE tracker per job.

Nine server scripts and two client scripts were written when a Job Tracker was
one EVENT. They each created a new tracker document per event, and the rollup
engine read those documents as the event stream. After the consolidation there
is one tracker per job and the events are child rows in `fps_updates`, so all
eleven have to change in the same migration -- a half-migrated site would record
nothing and derive nothing.

WHAT CHANGED IN EACH

  JO Emit Opened            opens THE tracker for a new job and appends its
                            first update, instead of creating a tracker per event
  JT Rollup                 reads evs from tracker.fps_updates instead of
  FPS Tracker Sweep         querying sibling Job Tracker documents
  CT BOE to Job Order       emit() appends an update row to the job's tracker
  POD Events (Save)         and saves the parent, which is what still triggers
  POD Events (Submit)       the rollup
  POD Events (Cancel)
  SI Events (Submit)
  SI Events (Cancel)
  Job Order - SOW Tracker UI  the Log update dialog and the Rebuild button call
                            fps_erpnext.api.trackers.log_update / .rebuild;
                            "Update history" opens the tracker instead of a
                            filtered list
  Job Order - Create Buttons  drops the "Job tracker note" button, which created
                            a second tracker for the job

HOW THESE BODIES WERE PRODUCED. Not by hand. Each one is the script that was
live on the site, transformed by a script, then parsed -- Python through
ast.parse, JavaScript through `node --check` -- and finally round-tripped
through this generated module and compared byte for byte against its source.
A transcription slip cannot reach the site through this path.

TWO ASSUMPTIONS WERE PROVED, NOT ASSUMED. The new rollup uses sorted() with a
lambda key, and the new emit() uses doc.append() and reads a child table through
the parent document. No script on this site had ever used sorted, lambda or any,
so a probe was run in this site's own safe_exec sandbox first and confirmed all
of them, along with doc.append and child access via the parent. The probe was
deleted afterwards. The last time something like this was assumed rather than
checked, six scripts failed silently for a day.

If a script is missing, or its body is not what was expected, it is SKIPPED and
logged rather than overwritten -- someone may have edited it since.

Safe to re-run: a script whose body already matches is left alone.
"""
...
try:
    ...
    tr = doc.name if frappe.db.exists("Job Tracker", doc.name) else frappe.db.get_value("Job Tracker", {"job_order": doc.name}, "name")
    if not tr:
        tr = frappe.get_doc({
...
def rollup(jo_name):
    ...
    tr_name = jo_name if frappe.db.exists("Job Tracker", jo_name) else frappe.db.get_value("Job Tracker", {"job_order": jo_name}, "name")
...
try:
    ...
    if jo:
        ...
        def emit(code, ref, text, visible=1, etype="System"):
            # Exact name first: the tracker IS named after the job order. If two
            # events ever race and a second tracker gets '<job>-2', every later
            # call still converges on the canonical one instead of splitting the
            # job's history across two.
            tr = jo if frappe.db.exists("Job Tracker", jo) else frappe.db.get_value("Job Tracker", {"job_order": jo}, "name")
            if not tr:
                tr = frappe.get_doc({
...
function relabel(frm) {
  ...
  frm.set_df_property('pod', 'label', transport_only ? 'Deliver to' : 'Destination / POD');
}

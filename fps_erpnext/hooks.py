...
doc_events = {
    ...
    "Customs Tracker": {
        "autoname": "fps_erpnext.api.trackers.name_from_job_order",
        # Derives the document-submission deadline (from the declaration date)
        # and the MOFA deadline (from the clearance date) on every save, so the
        # two dates can never drift out of step with the dates they are
        # calculated from. See fps_erpnext/api/customs_clock.py for the windows,
        # the "count from" dates and the colour thresholds.
